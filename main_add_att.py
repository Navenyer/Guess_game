...
def start_game(event=None): # Dodajemy event, by obsłużyć zdarzenie New Game
    global secret_number, attempts, MAX_ATTEMPTS
...
# Funkcja jest uproszczona do obslugi widoku, dynamiczne ustawienie trudnosci w start game
def set_difficulty():
    difficulty = difficulty_var.get()
    if difficulty == "easy":
